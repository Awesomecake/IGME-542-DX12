...
Mesh::Mesh(Vertex vertices[], unsigned int vertexNum, unsigned int indices[], unsigned int indexNum, Microsoft::WRL::ComPtr<ID3D12Device> device)
{
	indexCount = indexNum;
...
Mesh::Mesh(const wchar_t* filename, Microsoft::WRL::ComPtr<ID3D12Device> device) : indexCount(0)
{
...
void Mesh::CreateBuffers(Vertex* vertices, int vertexNum, unsigned int* indices, Microsoft::WRL::ComPtr<ID3D12Device> device)
{
	// Create the two buffers
	vertexBuffer = Graphics::CreateStaticBuffer(sizeof(Vertex), vertexNum, vertices);
	indexBuffer = Graphics::CreateStaticBuffer(sizeof(unsigned int), indexCount, indices);
	// Set up the views
	vbView.StrideInBytes = sizeof(Vertex);
	vbView.SizeInBytes = sizeof(Vertex) * vertexNum;
